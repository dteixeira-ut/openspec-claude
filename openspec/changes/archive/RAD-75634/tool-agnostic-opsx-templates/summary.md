...
### Why
The just-shipped single-source generator delivered the right architecture, but the template bodies were biased toward Claude — Cursor and Codex got working but degraded behavior, and `ScheduleWakeup` had no equivalent at all. Calling the package "cross-tool" while shipping Claude-flavored prose would mislead consumers of the planned `@usertestingenterprise/opsx` extraction. This change makes the templates honestly tri-tool.
