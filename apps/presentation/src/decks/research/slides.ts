...
└─ migrate-bff-foundation (integration)
  ...
  // ───────────────────────── Section 6 — WHAT'S NEXT (3 slides) ─────────────
  {
    id: 'next-followups',
    title: 'What\'s next — concrete follow-ups',
    density: 'both',
    body: [
      {
        type: 'numbered',
        items: [
          'Org-level service-starter repo for sibling-config parity (one source of truth for .prettierrc, eslint, tsconfig, Dockerfile patterns).',
          'Roll out npm run fmt as a CI gate across every TypeScript service in the org (one-line addition to the existing lint workflow).',
          'Run service-config-drift against every TS service in the org and triage findings as a coordinated cleanup.',
          'Graduate migrate-to-nestjs and service-config-drift out of this repo into a shared skills home.',
        ],
      },
      {
        type: 'callout',
        tone: 'evidence',
        content:
          'Each follow-up addresses a gap not closed by the two mitigation changes — these are scoped for a future RAD/PR.',
      },
    ],
    notes:
      "Three concrete asks. The service-starter repo is the biggest lift — it's the org-level mitigation that prevents sibling drift by construction.",
  },
  {
    id: 'next-open-questions',
    ...
    body: [
      {
        type: 'numbered',
        items: [
          'Does /opsx:plan\'s preflight catch the squash × stacked combo cleanly on the next migration?',
          'Do the new "non-code surfaces" and "delivery shape" sections feel like overhead, or rigor, in a smaller change (one capability, one PR)?',
          'Should domain skills move to a plugin registry now, or wait for a third skill to motivate the home?',
          'How does the framework hold up with two parallel agents working on different services (we tested parallel changes — not parallel agents)?',
          'What\'s the right cadence for "refresh the research deck" so it doesn\'t drift from new findings?',
        ],
      },
      {
        type: 'callout',
        tone: 'evidence',
        content:
          'Source: research notes §"Open questions to test next" (lines 217–223) — five questions, lightly updated for the post-mitigation state.',
      },
    ],
    notes:
      "Invite discussion here. The first question is the most testable; the last one is the most important if this deck becomes the canonical 'how does this approach work' artifact.",
  },
  {
    id: 'next-graduation',
    title: 'Graduating the domain skills',
    density: 'full',
    body: [
      {
        type: 'bullets',
        items: [
          'Today: skills/ lives in this repo, behind a clear "temporary home" framing.',
          'Next: invoke migrate-to-nestjs against a second consumer repo (different team, different service shape).',
          'Then: invoke service-config-drift across the TS-service fleet as a one-time audit.',
          'Outcome: skill bodies are battle-tested across ≥2 consumers and ready to graduate to a plugin or shared repo.',
        ],
      },
      {
        type: 'callout',
        tone: 'evidence',
        content:
          'Graduation criteria are documented in skills/README.md (see add-domain-skills change).',
      },
    ],
    notes:
      "The graduation path is the answer to 'why ship these here at all?' — they earn a home by proving they're useful outside the migration that birthed them.",
  },

  // ───────────────────────── Section 7 — CLOSE (1 slide) ─────────────
  {
    id: 'close-dogfooding',
